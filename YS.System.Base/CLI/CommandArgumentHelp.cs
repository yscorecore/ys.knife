namespace System.CLI
{
    ...
    public class CommandArgumentHelp
    {
        ...
        public string CommandLine
        {
            get
            {
                return System.CLI.CommandLine.Text;
            }
        }
        ...
        {
            return string.Format(parameter.Required ? "{0}{1} " : "[{0}{1}] ", System.CLI.CommandLine.CommandSeparators.First(), parameter.Command);
        }

        private static string FormatCommandLineParameter(CommandLineParameterAttribute parameter)
        {
            return !string.IsNullOrEmpty(parameter.ValueExample)
                       ? FormatValueExample(parameter)
                       : parameter.IsCommand()
                             ? FormatCommandArgument(parameter)
                             : FormatParameterArgument(parameter);
        }

        private static string FormatValueExample(CommandLineParameterAttribute parameter)
        {
            return string.Format(parameter.Required ? "{0} " : "[{0}] ", parameter.ValueExample);
        }
