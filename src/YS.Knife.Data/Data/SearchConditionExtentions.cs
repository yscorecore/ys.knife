...
namespace YS.Knife.Data
{
    ...
    public static class FilterInfoExtentions
    {
        ...
        public static IQueryable<T> WhereCondition<T>(this IQueryable<T> source, FilterInfo searchCondition)
        {
        ...
        {
            if (filterInfos == null || filterInfos.Length == 0) return source;
        ...
        public static Expression<Func<T, bool>> CreatePredicate<T>(this FilterInfo searchCondition)
        {
        ...
        private static Expression FromItemConditionItemInternal(Type entityType, FilterInfo searchItem, ParameterExpression p)
        {
            ...
            var converter = GetConvertByFilterType(searchItem.FilterType);
            return converter.ConvertValue(exp, pinfo, val);
        }
        private static Expression FromContidtionInternal(Type entityType, FilterInfo searchCondition, ParameterExpression p)
        {
    ...
    class GreaterThanExpressionConverter : ExpressionConverter
    {
        public override Expression ConvertValue(Expression p, PropertyInfo propInfo, object value)
        {
            if (value == null) throw new InvalidOperationException(string.Format("{0} 无法处理null值", FilterType.GreaterThan));
            ...
            {
                throw new InvalidOperationException(string.Format("{0} 只能处理实现了IComparable<{1}>接口的类型", FilterType.GreaterThan, ptype.FullName));
            }
