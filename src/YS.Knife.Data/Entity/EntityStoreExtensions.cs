...
namespace YS.Knife.Entity
{
    ...
    public static class EntityStoreExtensions
    {
        ...
        //#region ListLimit
        //public static PagedList<T> ListLimit<T>(this IEntityStore<T> store, Expression<Func<T, bool>> conditions, IEnumerable<OrderItem> orderItems, int offset, int limit)
        // where T : class
        //{
        //    if (orderItems == null) throw new ArgumentNullException(nameof(orderItems));
        //    if (orderItems.Count() <= 0) throw new ArgumentException(nameof(orderItems), "至少包含一个排序字段");
        ...
        //    var query = store.Query(conditions);
        //    var sortedQuery = query.Order(orderItems);
        //    return new PagedList<T>(sortedQuery, offset, limit);
        //}

        //public static PagedList<TR> ListLimit<T, TR>(this IEntityStore<T> store, Expression<Func<T, bool>> conditions, IEnumerable<OrderItem> orderItems, Func<IQueryable<T>, IQueryable<TR>> selector, int offset, int limit)
        //      where T : class
        //{
        ...
        //    var query = store.Query(conditions);
        //    var sortedQuery = selector(query).Order(orderItems);
        //    return new PagedList<TR>(sortedQuery, offset, limit);
        //}
        //#endregion
