...
namespace System.Linq
{
    public static  class LimitExtensions
    {
        ...
        public static PagedData<T> ToPagedData<T>(this IQueryable<T> source, int offset, int limit)
        {
            return source.ToPagedData(new LimitInfo(offset, limit));
        }
        public static PagedData<T> ToPagedData<T>(this IQueryable<T> source, ILimitInfo limitInfo)
        {
            _ = source ?? throw new ArgumentNullException(nameof(source));
            _ = limitInfo ?? throw new ArgumentNullException(nameof(limitInfo));
            var limit = limitInfo.Limit;
            ...
            var limitListData = source.Skip(offset).Take(limit).ToList();
            return new PagedData<T>(limitListData, offset, limit, totalCount);
        }
        #endregion
    }
}
