...
namespace YS.Knife.Generator.UnitTest
{
    ...
    public class AutoNotifyGeneratorTest
    {
        ...
        public void ShouldGenerateExpectCodeFile(string testCaseFileName)
        {
            ...
            var newComp = RunGenerators(CreateCompilation(codes.ToArray()), out var warningAndErrors,
        ...
        private static Compilation CreateCompilation(params string[] sources)
        {
            var allSources = sources
                .Select(p => CSharpSyntaxTree.ParseText(p, new CSharpParseOptions(LanguageVersion.CSharp10))).ToArray();
            return CSharpCompilation.Create("compilation",
                allSources,
                new[]
                {
                    MetadataReference.CreateFromFile(typeof(Binder).GetTypeInfo().Assembly.Location),
                    MetadataReference.CreateFromFile(typeof(INotifyPropertyChanged).GetTypeInfo().Assembly
                        .Location),
